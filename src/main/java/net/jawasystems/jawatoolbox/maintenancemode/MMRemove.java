    ...
    public boolean onCommand(CommandSender commandSender, Command cmnd, String string, String[] args) {
        ...
        if (args == null || args.length == 0){
            ...
            if (JawaToolBox.maintenanceList.contains(uuid)) {
                commandSender.sendMessage(ChatColor.GREEN + " > Removing user from the maintenance list and saving the new list.");
                
                JawaToolBox.maintenanceList.remove(uuid);
                
                FileHandler.saveMaintenanceList();
            } else {
                commandSender.sendMessage(ChatColor.RED + " > That user's UUID is not in the maintenance list.");
            }
        } else {
